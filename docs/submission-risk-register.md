...
| Source-vs-published drift | GitHub docs may describe readiness checks that are newer than `codex-oss-lens@latest`. | Application status, review FAQ, install smoke sample, release provenance, and npm publishing docs separate source-only evidence from published package behavior. | `npm run submission:versions`, `npm run release:provenance`, `npm run publish:samples` |
...
Do not submit if any local gate above fails, if the latest public CI is red, if npm latest no longer
matches the claimed release, or if the account owner has not reviewed the OpenAI organization ID and
terms in the submitting browser session.
