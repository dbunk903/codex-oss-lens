...
현재 v1.6.1 공개 릴리스와 npm 패키지가 있습니다. 의존성 없는 Node CLI와 정적 UI로 quota/token 분석, Git/workflow/outcome linking, API dry-run, doctor, CI, published install smoke test를 제공합니다. `submission-pack`, `form-draft`, `pack-validate`가 신청 evidence를 검증합니다.
